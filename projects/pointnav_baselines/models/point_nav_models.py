...
from allenact.embodiedai.preprocessors.BVFFModule.bvff_module import bvff_module
...
class ResnetTensorPointNavActorCritic(ActorCriticModel[CategoricalDistr]): # 在此处修改
    def __init__(
        self,
        action_space: gym.spaces.Discrete,
        observation_space: SpaceDict,
        ...
        if (
            ...
            self.resnet_preprocessor_uuid = resnet_preprocessor_uuid # 此处增加
        ...
        self.memory_key = "rnn"
        # 增加Brain-Visual融合模块
        self.bvff_module = bvff_module(fusion_method = 'cross_attn')
    ...
    def forward(  # type:ignore
        ...
        ### step1：获得视觉特征x和脑特征brain_feat
        visual_feat = self.goal_visual_encoder(observations) # [1, agent数量, 1568]
        brain_feat = observations[self.resnet_preprocessor_uuid]["brain_feat"]  # brain feat的维度 [1, agent数量, 768/384/512]
        # print("visual_feat's shape is", *visual_feat.shape)
        # print("brain_feat's shape is", *brain_feat.shape)
        
        ### step2：特征融合
        ## 融合1：cross attention融合 参考Cross-ViT
        x = self.bvff_module(brain_feat, visual_feat) # 采集数据时：[1, agent数量, 1568+brain_feat_dim] 更新权重时：[roll, agent数量, 1568+brain_feat_dim]
        # x = x.squeeze(1)
        # 融合2：直接融合
        # x = torch.cat([visual_feat, brain_feat], dim=-1) # [1, agent数量, 1568+brain_feat_dim]
        ## 融合3：FiLM融合 2025-07-03
        
        
        ### step3：输入到policy网络中
        x, rnn_hidden_states = self.state_encoder(
            x, memory.tensor(self.memory_key), masks
        ) # 此处已修改，输入由x改为x_joint_brain_feat或brain_visual_fusion_feat
        return (
            ActorCriticOutput(
                distributions=self.actor(x), values=self.critic(x), extras={}
            ),
...
class ResnetTensorGoalEncoder(nn.Module): # 此处修改
    def __init__(
        self,
        observation_spaces: SpaceDict,
        goal_sensor_uuid: str,
    ...
    def output_dims(self):
        if self.blind:
            ...
            return (
                2336
            ) # 此处已修改， 手动计算x_joint_brain_feat的维度 1568+512 = 2080  1568+1400=2968 1568+1600=3168 1568+896=2464 1568+768=2336
            # return (
            #     self.combine_hid_out_dims[-1]
            #     * self.resnet_tensor_shape[1]
            #     * self.resnet_tensor_shape[2]
    ...
    def compress_resnet(self, observations):
        resnet = observations[self.resnet_uuid]["resnet_feat"] # 此处已修改
        return self.resnet_compressor(resnet) 
    ...
    def adapt_input(self, observations):
        resnet = observations[self.resnet_uuid]["resnet_feat"] # ResNetPreprocessor x_joint_brain_feat_tensor = {"resnet_feat": x_layer4_output, "brain_feat": brain_feat} 此处已修改
        ...
        observations[self.resnet_uuid]["resnet_feat"] = resnet.view(-1, *resnet.shape[-3:]) # 此处已修改
        observations[self.goal_uuid] = observations[self.goal_uuid].view(-1, 2)
    ...
    def forward(self, observations):
        ...
        if self.blind:
            return self.embed_goal(observations[self.goal_uuid])
        embs = [
            self.compress_resnet(observations), # [batch_size, 32, 7, 7]
            self.distribute_target(observations), # [batch_size, 32, 7, 7]
        ]
        x = self.target_obs_combiner(torch.cat(embs, dim=1,)) # [batch_size, 32, 7, 7]
        x = x.reshape(x.size(0), -1)  # flatten [batch, 1568]
        return self.adapt_output(x, use_agent, nstep, nsampler, nagent)
